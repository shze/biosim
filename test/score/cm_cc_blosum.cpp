...
BOOST_AUTO_TEST_CASE(blosum_default_ctor) {
  ...
  score::cm_cc_blosum::dbl_triangular_matrix s(b62.get_dbl_bitscore_matrix());
  BOOST_CHECK_CLOSE(s(0, 0), 3.92912, 1e-3);
  BOOST_CHECK_CLOSE(s(1, 0), -1.413500, 1e-3);
  BOOST_REQUIRE_THROW(s(0, 1), std::out_of_range);
  score::cm_cc_blosum::int_triangular_matrix i(b62.get_int_bitscore_matrix());
  BOOST_CHECK(i(0, 0) == 4);
  BOOST_CHECK(i(1, 0) == -1);
  ...
  score::cm_cc_blosum b62_double(true);
...
BOOST_AUTO_TEST_CASE(blosum_frequency_ctor) {
  ...
  score::cm_cc_blosum b62_double("blosum62copy", b62_default.get_frequency_matrix(), true);
...
BOOST_AUTO_TEST_CASE(blosum_bitscore_ctor) {
  score::cm_cc_blosum b62_default;
  score::cm_cc_blosum b62("blosum62copy", b62_default.get_int_bitscore_matrix(), b62_default.get_bitscore_fraction());
