      ...
      qs file_psipredv::read(std::string const &__filename)
      {
        ...
        for( auto const &line: file_lines) {
          ...
          if( boost::regex_match( line, match, regex_psipredv_line)) {
            ...
            DEBUG << "Submatches: pos=" << pos << "; aa=" << aa_char << "; ss=" << ss_char
                << "; prob_C=" << prob_c << "; prob_H=" << prob_h << "; prob_E=" << prob_e;
                
            cc_sequence.emplace_back(aa_char);
            cchb_sequence.emplace_back(std::string(ss_char, 1), cchb::weight_map({{"C", prob_c}, {"H", prob_h}, {"E", prob_e}}));
          } // if
        } // for
